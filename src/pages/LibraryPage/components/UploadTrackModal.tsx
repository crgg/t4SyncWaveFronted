...
export function UploadTrackModal({ isOpen, onClose }: UploadTrackModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await uploadService.uploadAudioToLibrary(selectedFile);

      // Invalidate library query to refresh the list
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-green-600/80 dark:text-green-400/80 mt-1">
                    {uploadedFile.name} has been added to your library
                  </p>
                </div>
              </div>
            </motion.div>
